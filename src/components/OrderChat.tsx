 ...
 *
 * Photos live in the private order-chat-photos bucket; a message stores the
 * object key and this component swaps it for a short-lived signed URL.
 */
export function OrderChat({ orderId, userId }: { orderId: string; userId: string }) {
  ...
  async function send() {
    ...
    const { error: sendError } = await supabase
      .from("messages")
      .insert({ order_id: orderId, sender_id: userId, body, photo_path: photoPath });
    setSending(false);
    if (sendError) {
    ...
    <div className="mt-3 rounded-2xl bg-white p-3" style={{ color: "var(--kb-ink)" }}>
      ...
      <div ref={listRef} className="max-h-72 space-y-2 overflow-y-auto pr-1">
        {messages.length === 0 ? (
          ...
          messages.map((m) => {
            // Rider (you) on the right, customer on the left.
